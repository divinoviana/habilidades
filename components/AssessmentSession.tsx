  ...
  const handleFinish = async () => {
    ...
    if (!isMock) {
      await supabase.from('assessments').insert([{
        ...
        subject,
        quarter: 1, // Ajustável conforme settings
        grade: currentUser.grade,
        questions,
        answers, // Importante para visualização do professor
        score,
        is_mock: false,
        feedback,
    ...
    return (
      <div className="fixed inset-0 bg-slate-900 z-[100] flex items-center justify-center p-6 text-center">
        <div className="max-w-md bg-white rounded-[40px] p-10 shadow-2xl">
          ...
          <p className="text-slate-50 mb-8">Violação das regras detectada.</p>
          <button onClick={onCancel} className="w-full bg-slate-900 text-white font-black py-4 rounded-2xl">Voltar</button>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-4xl mx-auto py-12 px-4 animate-fade-in">
        <div className="bg-white rounded-[48px] p-12 shadow-xl border border-slate-200 text-center">
          ...
          <div className="flex justify-center gap-12 my-10 bg-slate-50 p-8 rounded-[32px]">
            <div><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Nota</p><p className="text-5xl font-black text-blue-600">{totalScore.toFixed(1)}</p></div>
            <div className="w-px bg-slate-200" />
            <div><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Acertos</p><p className="text-5xl font-black text-slate-800">{answers.filter((a, i) => a === questions[i].correctIndex).length}/{questions.length}</p></div>
          </div>

          <div className="text-left bg-blue-600 p-8 rounded-[32px] text-white shadow-xl">
            <h3 className="font-black flex items-center gap-2 mb-4 text-xs uppercase tracking-widest"><Sparkles size={16}/> Feedback da IA</h3>
            {loadingFeedback ? <p className="animate-pulse">Sincronizando resultados...</p> : <p className="text-blue-50 leading-relaxed text-sm italic">"{aiFeedback}"</p>}
          </div>

          <button onClick={() => onComplete({ subject, score: totalScore, isMock })} className="mt-10 bg-slate-900 text-white font-black px-12 py-5 rounded-[24px] shadow-xl">RETORNAR AO PAINEL</button>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-5xl mx-auto pb-32 pt-10 px-4">
      <div className="bg-white/80 backdrop-blur-md border border-slate-200 rounded-[32px] p-6 mb-8 flex justify-between items-center shadow-lg sticky top-10 z-50">
        <div className="flex items-center gap-6">
          ...
          <div><h2 className="font-black text-slate-800 text-lg">{subject}</h2><p className="text-[10px] uppercase font-black text-blue-500 tracking-widest">{isMock ? 'Simulado' : 'Avaliação Oficial'}</p></div>
        </div>
      </div>
        ...
        <div className="bg-white border border-slate-200 rounded-[40px] p-10 shadow-sm relative overflow-hidden">
          {q.citation && (
            <div className="mb-8 border-l-4 border-blue-600 pl-8 space-y-4">
              <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest flex items-center gap-2"><Quote size={12}/> Texto Base</p>
          ...
          {q.visualDescription && (
            <div className="mb-8 bg-slate-50 rounded-[24px] p-8 border border-slate-100 space-y-3">
              ...
              <div className="p-6 bg-white border border-dashed border-slate-200 rounded-xl text-slate-500 text-sm italic">{q.visualDescription}</div>
            </div>
          )}
          <h3 className="text-2xl font-black text-slate-800 mb-10">{q.text}</h3>
          <div className="space-y-4">
            {q.options.map((opt, i) => (
              <button key={i} onClick={() => { const n = [...answers]; n[currentIdx] = i; setAnswers(n); }} className={`w-full text-left p-6 rounded-[24px] border-2 transition-all flex items-start gap-5 ${answers[currentIdx] === i ? 'bg-blue-600 border-blue-600 text-white shadow-xl scale-[1.01]' : 'bg-white border-slate-100 hover:border-blue-200 text-slate-600'}`}>
